...
do k = 1,mzp
   ...
   !CHECK BROWN CARBON MODES
   if(ibrnc > 0)then
     if(isnanr(micro_g(ngrid)%brc1np(k,i,j)) .or. &
        isnanr(micro_g(ngrid)%brc1mp(k,i,j)) .or. &
        isnanr(micro_g(ngrid)%brc2np(k,i,j)) .or. &
        isnanr(micro_g(ngrid)%brc2mp(k,i,j)) ) prtflg=1
   endif
 ...
 if(prtflg==1)then
    ...
    if(ibrnc > 0)then
      print*,'brc1np:         ',micro_g(ngrid)%brc1np(k,i,j)
      print*,'brc1mp:         ',micro_g(ngrid)%brc1mp(k,i,j)
      print*,'brc2np:         ',micro_g(ngrid)%brc2np(k,i,j)
      print*,'brc2mp:         ',micro_g(ngrid)%brc2mp(k,i,j)
    endif
    if(isalt > 0)then
