...
do j = 1,m3
     ...
     do k = 1,m1
       ...
       if(zerocheck==1 .and. iccnlev>=2 .and. cnmhx(k,lcat)>0.0) then
         ...
         if(cxloss > 0.0) then
           ...
           rg=((0.23873/aero_rhosol(aerocat)*cnmhx(k,lcat)/max(1.e-10,cxloss))**(0.3333))/aero_rg2rm(aerocat)
         ...
         cnmhx_num = cnmhx(k,lcat) * (0.23873/aero_rhosol(aerocat)) / ((rg * aero_rg2rm(aerocat)) ** 3.)
 ...
 do k = 1,m1
  ...
  if(ibrnc>0) then
   if(micro%brc1np(k,i,j) > maxaero) then
     print*,"Too many Brown Carbon 1:",micro%brc1np(k,i,j)
     toomany=1
   endif
   if(micro%brc2np(k,i,j) > maxaero) then
     print*,"Too many Brown Carbon 2:",micro%brc2np(k,i,j)
     toomany=1
   endif
  endif
  if(isalt>0) then
  ...
  if(ibrnc>0) then
   if(micro%brc1np(k,i,j)<mincon .or. micro%brc1mp(k,i,j)<minmas) then
      micro%brc1np(k,i,j) = 0.0
      micro%brc1mp(k,i,j) = 0.0
   endif
   if(micro%brc2np(k,i,j)<mincon .or. micro%brc2mp(k,i,j)<minmas) then
      micro%brc2np(k,i,j) = 0.0
      micro%brc2mp(k,i,j) = 0.0
   endif
  endif
  if(isalt>0) then
