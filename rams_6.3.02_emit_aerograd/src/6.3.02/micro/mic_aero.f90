...
!BEE Testing CCN source fxn 2021-08-21

!CALL ccn_sources (mzp,mxp,myp,ia,iz,ja,jz        &
!                    ,basic_g(ngrid)%dn0          &
!                    ,micro_g(ngrid)%cccnp(1,i,j) &
!                    ,micro_g(ngrid)%cccmp(1,i,j) & 
!                    )

!BEE Testing Brown Carbon source fxn 2022-11-23

CALL brc_sources (mzp,mxp,myp,ia,iz,ja,jz        &
                    ,basic_g(ngrid)%dn0          &
                    ,micro_g(ngrid)%brc1np(1,i,j) &
                    ,micro_g(ngrid)%brc1mp(1,i,j) & 
                    )
 ...
 do j = ja,jz
  do i = ia,iz
   ...
   CALL aero_copy (1,mzp &
    ...
    ,micro_g(ngrid)%brc1np(1,i,j),micro_g(ngrid)%brc1mp(1,i,j) &
    ,micro_g(ngrid)%brc2np(1,i,j),micro_g(ngrid)%brc2mp(1,i,j))
...
if(aflag==1)then
 ...
 do k = 1,m1-1
   ...
   if (ibrnc > 0) then
     aerocon(k,8) = brc1np(k)
     aeromas(k,8) = brc1mp(k)
     aerocon(k,9) = brc2np(k)
     aeromas(k,9) = brc2mp(k)
   endif
 enddo
...
elseif(aflag==2)then
 ...
 do k = 1,m1-1
   ...
   if (ibrnc > 0) then
    brc1np(k) = aerocon(k,8)
    brc1mp(k) = aeromas(k,8)
    brc2np(k) = aerocon(k,9)
    brc2mp(k) = aeromas(k,9)
   endif
 enddo
endif
