!##############################################################################
Subroutine ccn_sources (m1,m2,m3,ia,iz,ja,jz,dn0       &
                       ,cccnp,cccmp)
...
real, dimension(m1,m2,m3) :: cccnp,cccmp,dn0
real :: ccn_maxt, ccn_a, ccn_b

!Convert RAMSIN #/mg to #/kg
 ccn_maxt = ccn_max * 1.e6 
 !ccn_a = 500.*1.e6/(24.*3600.) !6.*3600.
 !ccn_b = 450.*1.e6/(24.*3600.)
 ccn_a = 275.*1.e6/(24.*3600.)
do j = ja,jz
 do i = ia,iz
  do k = 1,m1
   !source in first 10 levelsa
   !nogradient
   if(k<=2) cccnp(k,i,j)=cccnp(k,i,j)+ccn_a
   !source exponential decay
   if(k>2)  cccnp(k,i,j)=cccnp(k,i,j)+(ccn_a*exp(-zt(k)/7000.))
...
return
END SUBROUTINE ccn_sources
!##############################################################################

Subroutine brc_sources (m1,m2,m3,ia,iz,ja,jz,dn0       &
                       ,brc1np,brc1mp)
...
integer :: m1,m2,m3,ia,iz,ja,jz,i,j,k
real, dimension(m1,m2,m3) :: brc1np,brc1mp,dn0
real :: brc_maxt, brc_a, brc_b

!Convert RAMSIN #/mg to #/kg
brc_a = 550.*1.e6/(24.*3600.)
brc_b = 450.*1.e6/(24.*3600.)
...
do j = ja,jz
 do i = ia,iz
  do k = 1,m1
   !source in first 10 levelsa
   !nogradient
   if(k<=2) brc1np(k,i,j)=brc1np(k,i,j)+brc_a - brc_b*cos((float(j+mj0(1))/1000.)*atan(1.0)*8.)
   !source exponential decay
   if(k>2)  brc1np(k,i,j)=brc1np(k,i,j)+(brc_a - brc_b*cos((float(j+mj0(1))/1000.)*atan(1.0)*8.))*exp(-zt(k)/7000.)
 
   !Set up Field of Brown Carbon mass mixing ratio (kg/kg)
