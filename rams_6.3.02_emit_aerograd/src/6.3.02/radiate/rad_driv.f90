   ...
   if (iswrtyp .eq. 3 .or. ilwrtyp .eq. 3) then
      ...
      if (level <= 2 .or. level==4) then
           ...
           CALL radcomp3 (mzp,mxp,myp,ia,iz,ja,jz  &
            ...
            ,micro_g(ngrid)%brc1np     (1,1,1)  &
            ,micro_g(ngrid)%brc1mp     (1,1,1)  &
            ,micro_g(ngrid)%brc2np     (1,1,1)  &
            ,micro_g(ngrid)%brc2mp     (1,1,1))
      endif
...
Subroutine radcomp3 (m1,m2,m3,ia,iz,ja,jz  &
   ...
   ,salt_spum_np,salt_spum_mp,brc1np,brc1mp,brc2np,brc2mp)
   ...
   do i = ia,iz
      ...
      if(iaerorad==1 .and. level .ne. 4)then
        CALL aero_copy (1,m1 &
           ...
           ,brc1np(1,i,j),brc1mp(1,i,j) &
           ,brc2np(1,i,j),brc2mp(1,i,j))
      endif
