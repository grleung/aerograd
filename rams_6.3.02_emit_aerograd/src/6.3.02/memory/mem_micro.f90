...
! Allocate arrays based on options (if necessary)
      if (level >= 0 .and. level .ne. 4) then
         ...
         if(ibrnc > 0) then
            allocate (micro%brc1np(n1,n2,n3))
            allocate (micro%brc2np(n1,n2,n3))
            allocate (micro%brc1mp(n1,n2,n3))
            allocate (micro%brc2mp(n1,n2,n3))
         endif
      endif
...
implicit none
   ...
   if (allocated(micro%brc1np))   deallocate (micro%brc1np)
   if (allocated(micro%brc2np))   deallocate (micro%brc2np)
   if (allocated(micro%brc1mp))   deallocate (micro%brc1mp)
   if (allocated(micro%brc2mp))   deallocate (micro%brc2mp)
...
!Aerosol categories mass and number
   ...
   if (allocated(micro%brc1np)) &
      CALL vtables2 (micro%brc1np(1,1,1),microm%brc1np(1,1,1)  &
                 ,ng, npts, imean,  &
                 'BRC1NP :3:anal:mpti:mpt1')
   if (allocated(micro%brc2np)) &
      CALL vtables2 (micro%brc2np(1,1,1),microm%brc2np(1,1,1)  &
                 ,ng, npts, imean,  &
                 'BRC2NP :3:anal:mpti:mpt1')
   if (allocated(micro%brc1mp)) &
      CALL vtables2 (micro%brc1mp(1,1,1),microm%brc1mp(1,1,1)  &
                 ,ng, npts, imean,  &
                 'BRC1MP :3:anal:mpti:mpt1')
   if (allocated(micro%brc2mp)) &
      CALL vtables2 (micro%brc2mp(1,1,1),microm%brc2mp(1,1,1)  &
                 ,ng, npts, imean,  &
                 'BRC2MP :3:anal:mpti:mpt1')
