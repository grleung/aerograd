...
if(level<=3)then
 if(iaerosol>0 .or. idust>0 .or. isalt>0 .or. ibrnc>0) CALL aerosol_init ()
endif
...
!     on all grids.
   ...
   do ifm=1,ngrids
      ...
      if((initial==1 .or. initial==2 .or. (initial==3.and.iaerohist==1)) &
         .and. level<=3) then
       if(iaerosol > 0) CALL init_ccn (mzp,mxp,myp,i0,j0    &
       ...
       if(idust  > 0)  CALL init_dust (mzp,mxp,myp   &
          ...
          ,basic_g(ifm)%dn0   (1,1,1),ifm)
       if(ibrnc  > 0)  CALL init_brown_carbon (mzp,mxp,myp,i0,j0   &
          ,micro_g(ifm)%brc1np (1,1,1)  &
          ,micro_g(ifm)%brc2np (1,1,1)  &
          ,micro_g(ifm)%brc1mp (1,1,1)  &
          ,micro_g(ifm)%brc2mp (1,1,1)  &
          ,basic_g(ifm)%dn0   (1,1,1),ifm)
