...
!********************************************************************
 ...
 if (ibrnc .lt. 0 .or. ibrnc .gt. 1) THEN
    print*,'FATAL - IBRNC OUT OF RANGE: MUST BE 0-1'
    IFATERR = IFATERR + 1
 endif
...
!ALLOWABLE RANGE OF MICRO / RADIATION FLAGS
 ...
 if (icloud .eq. 5) then
  if (iaerosol.eq.0 .and. idust.eq.0 .and. isalt.eq.0 .and. ibrnc.eq.0) then
    print*,'FATAL - If icloud==5, then either IAEROSOL, IDUST, IBRNC, or ISALT'
...
elseif (level .eq. 4) then
   ...
   if (icloud      > 0 .or. &
    ...
    print*,'IBRNC, ISALT, IDUST, ICCNLEV, IAERORAD, IAERODEP, IAEROPRNT' 
